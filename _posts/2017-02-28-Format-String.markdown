...
Aujourd'hui un petit article qui traitera d'un cas simple de Format String où nous exploiterons un buffer passé en argument.    
L'idée de cet article fait suite à la machine [Pegasus 1](https://www.vulnhub.com/entry/pegasus-1,109/) de vulnhub que je vous recommande chaudement. J'essaierai d'être assez pédagogique, le cas ci-dessous étant un cas basique sans contournement des protections de la pile.  
...
Here we go, nous trouvons le premier élement de notre buffer, A (**%6$x** nous affiche la 6ème adresse). **41**c10000 reflète en réalité ce qui est stocké en mémoire de la façon suivante: 0000c141. En cause l'architecture petit-boutiste des processeurs x86. En effet le formateur %x lira le bloque de 32 bits comme une adresse, affichant en premier les octets de poids fort (donc stockés le plus à droite).  
...
objdump est une command qui nous fournit diverse informations sur les fichiers objet.
...
Maintenant la partie sensible, l'écriture de cette adresse. Pour écrire cette adresse nous utiliserons le formateur **%7$hn** dans notre cas. L'instruction de formatage %hn n'écrit que sur 16 octet.  
...
Pourquoi ne pas écrire directement les 32 bits? Nous utiliserons la précision sur le formatage d'une valeur, ici le nombre de caractères sur lequel nous l'afficherons. L'adresse convertie en entier fournie un entier tout simplement trop grand pour être utilisé.   
```
...
Allons faire notre marché sur [shell-storm](http://shell-storm.org/shellcode/) (un peu de chauvinisme ne fait pas de mal).  
Celui la est fun: http://shell-storm.org/shellcode/files/shellcode-872.php. Un shellcode qui appelle netcat (/bin/nc) et bind le port 17771 pour le rediriger sur /bin/sh.  
...
Pour conclure, si vous rencontrer un souci entre l'exécution dans l'environnement gdb et votre shell, voici une réponse: [https://stackoverflow.com/questions/17775186/buffer-overflow-works-in-gdb-but-not-without-it](https://stackoverflow.com/questions/17775186/buffer-overflow-works-in-gdb-but-not-without-it). Assurez que l'environnement soit strictement identique.
  
J'espère que cela vous aura plu, n'hésitez pas pour toute question à me contacter ou laisser un com.
See Ya!