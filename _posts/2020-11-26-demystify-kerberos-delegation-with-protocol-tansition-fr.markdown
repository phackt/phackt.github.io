...
published: false
---  
*P.S: an english version will be released soon.*    

Bonjour,  
  
Aujourd'hui nous allons parler d'une attaque qui implique les extensions de protocole Kerberos ```S4U2Self``` et ```S4U2Proxy```.  
...
```
TODO Get-ADObject
lien sur Invoke-Recon
Résultats d'invoke-recon sur le sujet
```
...
Cette délégation va faire intervenir les extensions de protocole **ServiceForUserToSelf** et **ServiceForUserToProxy** :  
 ...
 - **S4U2Proxy** va utiliser *T,sa* comme preuve de l'authentification de *whatever* auprès de *SA* et ainsi récupérer un ticket de service *forwarded* pour *SB*.
